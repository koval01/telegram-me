...
class GeminiService:
    """A service class for interacting with the Google Gemini AI model.

    This class provides methods to generate content using the Gemini AI model
    with various configurations and safety settings. It supports multiple
    languages and allows customization of the model behavior.

    Attributes:
        client: The Gemini AI client instance.
        model: The default model to use for content generation.
        safety_settings: List of safety settings for content filtering.
        system_instructions: Dictionary of system instructions for different languages.
    """

    def __init__(self):
        """Initialize the GeminiService with default settings and API key."""
    ...
    async def generate_content(
        ...
        *,
        ...
        Args:
            lang: The language for system instructions (default: "en").
            model: The model to use for generation (uses default if None).
            contents: The input content to process.
            config: Optional configuration override for generation.

        Returns:
            GenerateContentResponse: The response from the Gemini AI model.

        Note:
            If no config is provided, uses default configuration with:
            - temperature=0.8
            - response_mime_type="text/plain"
            - predefined safety settings
            - system instruction for the specified language
        """
        ...
        generate_content_config = GenerateContentConfig(
            ...
            system_instruction=self.system_instructions.get(lang)
        )
