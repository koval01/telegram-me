...
class GenerateResponse:
    ...
    async def _process_media_item(
        ...
        try:
            ...
            logger.warning("Unsupported media type: %s", mime)
            return None
        except Exception as e:  # pylint: disable=broad-exception-caught
            logger.error("Error processing media %s: %s", url, str(e))
            return None
    ...
    async def generate(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Unexpected error generating response: %s", str(e))
            raise RuntimeError("Failed to generate AI response") from e
